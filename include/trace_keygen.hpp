...
void AutoIdComposeTensor(const uint32_t M, const vector<usint>& indices_src1,
...
void MergeAutoIds(const uint32_t M, const vector<vector<usint>>& base_indices,
  ...
  for (uint32_t i = 0; i < num_pack; ++i) merge_ids[i] = base_indices[i + beg];

  uint32_t lim = num_pack - 1;
  for (uint32_t i = 0, j = 0; j < lim; i = i + 2, ++j) {
    //    cour << "j=" << j << endl;
    //    cour << "ACcess to " << 2*j  << " " << 2*j + 1 << " " << num_pack + j
    //    << endl;
    AutoIdComposeTensor(M, merge_ids[2 * j], merge_ids[2 * j + 1],
                        merge_ids[num_pack + j]);
  }
...
template <class T>
void TraceKeyGen(lbcrypto::CryptoContext<lbcrypto::DCRTPoly>& cc,
                 const size_t numitr, T& keys, vector<usint>& index_list,
  ...
  if (is_evalsums) {
    ...
    g = gsign;
    index_list.push_back(g);
    auto eval_automorph_ks =
        cc->EvalAutomorphismKeyGen(keys.secretKey, index_list);
...
// Pure Unroll Keygen
template <class T>
double EvalSumKeyGenUnroll(CryptoContext<DCRTPoly>& cc, T& keys,
...
#ifdef DEBUG
  std::cout << " #loop (" << num_loop << ") is divided into " << h
            << " chunks with base chunk size " << unroll_base << " (repeating "
            << num_repeat_base << " times) and " << unroll_rem << " (repeating "
            << num_repeat_rem << " times)" << std::endl;
#endif
  ...
  for (size_t i = 0; i < num_repeat_rem; ++i) {
    ...
    MergeAutoIds(M, base_indices, beg, end,
                 vec_auto_indices_par[num_repeat_base + i]);
  }

#ifdef DEBUG
  std::cout << "Indices (including 1):[\n";
  for (auto vec : vec_auto_indices_par) {
    std::cout << "  [";
    for (auto a : vec) std::cout << a << " ";
    std::cout << "] #keys = " << vec.size() << end;
  }
#endif
...
void GenZmstar(uint32_t logN, vector<uint32_t>& odds,
               vector<int>& is_consumed) {
  uint32_t N = 1UL << logN;
  uint32_t M = 2 * N;
...
size_t GenUnroll(const uint32_t logN, const uint32_t M, const size_t num_unroll,
  ...
  std::cout << std::endl;
  for (auto chunk : chunk_sizes) std::cout << chunk << " ";
  std::cout << std::endl;
  uint32_t mm = M - 1;
  ...
  size_t numkeys = 0;
  for (auto v : vec_auto_indices) numkeys += v.size();
  return numkeys;
}

double TraceSeqHKOffline(const size_t logN, const size_t num_itr,
                         const uint32_t multDepth,
                         const uint32_t scaleFactorBits, const uint32_t dnum,
  ...
  cc = CryptoContextFactory<DCRTPoly>::genCryptoContextCKKS(
      multDepth, scaleFactorBits, batchSize, securityLevel, 1 << logN, rsTech,
      ksTech, dnum);

  uint32_t N = cc->GetRingDimension();
  std::cout << "CKKS scheme is using ring dimension " << N
            << " logN=" << std::log2(N) << std::endl
            << endl;
