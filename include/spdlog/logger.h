...
class SPDLOG_API logger {
 public:
  ...
  template <typename... Args>
  void log(level::level_enum lvl, fmt::format_string<Args...> fmt,
           Args &&...args) {
...
#ifdef SPDLOG_WCHAR_TO_UTF8_SUPPORT
  template <typename... Args>
  void log(level::level_enum lvl, fmt::wformat_string<Args...> fmt,
           Args &&...args) {
    log(source_loc{}, lvl, fmt, std::forward<Args>(args)...);
  }
  ...
  template <typename... Args>
  void log(source_loc loc, level::level_enum lvl,
           fmt::wformat_string<Args...> fmt, Args &&...args) {
    log_(loc, lvl, fmt, std::forward<Args>(args)...);
  }
  ...
  void log_(source_loc loc, level::level_enum lvl, wstring_view_t fmt,
    ...
    SPDLOG_TRY {
      ...
      fmt::detail::vformat_to(
          wbuf, fmt, fmt::make_format_args<fmt::wformat_context>(args...));
      memory_buf_t buf;
